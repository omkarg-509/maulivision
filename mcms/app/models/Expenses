...
class Expenses extends Database
{
    ...
    public function allByAdmin($adminId, $from = null, $to = null)
    {
        ...
        $sql = "SELECT id, type, method, amount, note, entry_date, created_at FROM mcms_expenses WHERE vid = ?$filter ORDER BY entry_date DESC, id DESC";
    ...
    {
        $stmt = $this->db->prepare("INSERT INTO mcms_expenses (vid,type,method,amount,note,entry_date) VALUES (?,?,?,?,?,?)");
    ...
    {
        $stmt = $this->db->prepare("DELETE FROM mcms_expenses WHERE id = ? AND vid = ?");
    ...
    public function dailyStats($adminId, $from, $to)
    {
        ...
        $sql = "SELECT entry_date AS day,
            ...
            FROM mcms_expenses
            WHERE vid = ? AND entry_date BETWEEN ? AND ?
    ...
    public function summaryTotals($adminId, $from, $to)
    {
        ...
        $sql = "SELECT 
            ...
            SUM(CASE WHEN type='borrow' THEN amount ELSE 0 END) borrow,
            SUM(CASE WHEN type='repay' THEN amount ELSE 0 END) repay
        FROM mcms_expenses WHERE vid=? AND entry_date BETWEEN ? AND ?";
        $stmt = $this->db->prepare($sql);
        $stmt->bind_param('iss', $adminId, $from, $to);
        $stmt->execute();
