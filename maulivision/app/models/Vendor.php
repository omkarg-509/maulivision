...
class Vendor extends Database
{
    ...
    public function create($adminId,$fullName,$phone,$email,$username,$password,$bName,$bRole,$bNumber,$bAddress)
    {
        $stmt=$this->db->prepare("INSERT INTO vendors (admin_id,full_name,phone,email,username,password,business_name,business_role,business_number,business_address) VALUES (?,?,?,?,?,?,?,?,?,?)");
        $stmt->bind_param('isssssssss',$adminId,$fullName,$phone,$email,$username,$password,$bName,$bRole,$bNumber,$bAddress);
